...
final class Notes extends VKAPIRequestHandler
{
    ...
    function editComment(int $comment_id, int $owner_id = 0, string $message)
    {
        $comment = (new CommentsRepo)->get($comment_id);
    ...
    function get(int $user_id, string $note_ids = "", int $offset = 0, int $count = 10, int $sort = 0)
    {
        ...
        $notes = iterator_to_array((new NotesRepo)->getUserNotes((new UsersRepo)->get($user_id), 1, $count, $sort == 0 ? "ASC" : "DESC"));
        $nodez = (object) ["count" => (new NotesRepo)->getUserNotesCount((new UsersRepo)->get($user_id)), "notes" => []];
    ...
    function getComments(int $note_id, int $owner_id, int $sort = 1, int $offset = 0, int $count = 100)
    {
        ...
        if($note->isDeleted())
            $this->fail(189, "Note is deleted");
        $arr = (object) ["count" => $note->getCommentsCount(), "comments" => []];
